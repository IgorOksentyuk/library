...
function addContent(content, container) {
    if (!container.classList.contains("press")) {
        for (let i = 0; i < content.length; i++) {
            ...
            p.innerHTML = `${i + 1}. ${content[i].firstName} ${content[i].lastName} - ${content[i].title}`
...
buttons[0].addEventListener("click", function() {
    const content = books.sort((a, b) => a.title > b.title ? 1 : -1);
...
buttons[1].addEventListener('click', function() {
    const content = books.sort((a, b) => a.lastName > b.lastName ? 1 : -1);
    const container = lists[0];
    ...
    if (!container.classList.contains("press")) {
        for (let i = 0; i < content.length; i++) {
            const p = document.createElement('p');
            p.innerHTML = `${i + 1}. ${content[i].lastName} ${content[i].firstName} - ${content[i].title}`
            container.appendChild(p);
        }
        container.classList.add("press");
    }
...
buttons[2].addEventListener('click', function() {
    const content = books.sort((a, b) => b.rating > a.rating ? 1 : -1);
    const container = lists[0];
    ...
    if (!container.classList.contains("press")) {
        for (let i = 0; i < content.length; i++) {
            const p = document.createElement('p');
            p.innerHTML = `${i + 1}. ${content[i].firstName} ${content[i].lastName} - ${content[i].title} - ${content[i].rating}`
            container.appendChild(p);
        }
        container.classList.add("press");
    }
